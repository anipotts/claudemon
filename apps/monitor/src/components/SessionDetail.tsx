...
// ── Module-level agent open state — survives component re-mounts ───
const _agentOpenStates = new Map<string, Map<string, boolean>>();
function getSessionAgentState(sessionId: string): Map<string, boolean> {
  if (!_agentOpenStates.has(sessionId)) _agentOpenStates.set(sessionId, new Map());
  return _agentOpenStates.get(sessionId)!;
}
  ...
  return (
    <For each={props.actions}>
      {(action) => {
        ...
        return (
          <div class="mx-2 mt-2 mb-3 rounded border border-suspicious/40 bg-[#c9a96e0a] action-banner overflow-hidden">
            ...
            <Show when={richContext()}>
              {(ctx) => (
                <div class="border-t border-suspicious/15 px-3 py-2 max-h-[500px] overflow-y-auto bg-[#0a0a0a]/40">
                  ...
                  <Show when={ctx().type === "notification"}>
                    ...
                    <Show when={(ctx() as { plan: string }).plan}>
                      <div class="mt-2 border-t border-panel-border/20 pt-2 max-h-[400px] overflow-y-auto">
                        <MarkdownBlock text={(ctx() as { plan: string }).plan} maxLength={8000} />
                      </div>
                    </Show>
                  </Show>
                </div>
...
export const SessionDetail: Component<{
  ...
  // Per-agent open/close state — module-level, survives component re-mounts
  const [agentOpenVersion, setAgentOpenVersion] = createSignal(0);
  const agentState = () => getSessionAgentState(s().session_id);
  const getAgentOpen = (agentId: string, defaultOpen: boolean) => {
    agentOpenVersion(); // track reactivity
  ...
  const toggleAgentOpen = (agentId: string) => {
    ...
    setAgentOpenVersion((v) => v + 1); // trigger re-render
  };
